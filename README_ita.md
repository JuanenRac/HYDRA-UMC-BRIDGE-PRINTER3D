...
- **[HYDRA-UMC-MQTT-BROKER](https://github.com/JuanenRac/HYDRA-UMC-MQTT-BROKER)** — il trasporto reale di `mqtt_transport.py` per i topic `hydra/bridges/printer3d/...` propri di questo ponte (stato, vero start/pause/resume/cancel di Moonraker, la porta di lavoro condivisa) - vedi il `docs/BRIDGE_TOPICS.md` di quel repository.
