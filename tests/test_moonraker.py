...
class MoonrakerTests(unittest.TestCase):
    ...
    def test_unparseable_print_stats_response_fails_closed(self):
        MoonrakerFixtureHandler.print_stats_response_override = json.dumps({"result": {"status": {}}}).encode("utf-8")
        status = MoonrakerProbe().fetch(f"http://127.0.0.1:{self.server.server_port}")
        self.assertEqual(status.state, MachineState.OFFLINE)
    ...
    def test_parse_print_stats_unwraps_the_real_result_envelope_regression_for_print_02(self):
        ...
        # real shape - before the fix this returned OFFLINE/"missing
        # print_stats" for a perfectly healthy standby printer.
        real_shape = {"result": {"status": {"print_stats": {"state": "standby"}}}}
        self.assertIsNone(MoonrakerProbe.parse_print_stats(real_shape))
        printing = {"result": {"status": {"print_stats": {"state": "printing"}}}}
        status = MoonrakerProbe.parse_print_stats(printing)
        self.assertEqual(status.state, MachineState.RUNNING)
...
class MoonrakerJobControlTests(unittest.TestCase):
    ...
    def test_start_job_refuses_an_abort_phase_job_regression_for_print_01(self):
        ...
        result = MoonrakerJobControl().start_job(self.job(JobPhase.ABORT), CellState.FAULT, faulted, self.base_url, "part.gcode")
        self.assertFalse(result.allowed)
        self.assertFalse(result.executed)
        self.assertEqual(MoonrakerFixtureHandler.requested_paths, [])

        idle = PrinterStatus(MachineState.IDLE, "printer ready")
        also_refused = MoonrakerJobControl().start_job(self.job(JobPhase.ABORT), CellState.READY, idle, self.base_url, "part.gcode")
        self.assertFalse(also_refused.allowed)
        self.assertEqual(MoonrakerFixtureHandler.requested_paths, [])
