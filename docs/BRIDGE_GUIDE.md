...
`mqtt_transport.py` reaches this same real logic over `HYDRA-UMC-MQTT-BROKER`, this bridge's second real transport. `PrinterMqttBridge.handle_message()` routes `hydra/bridges/printer3d/cmd/{status,job,start,pause,resume,cancel}` to the same `MoonrakerProbe`/`MoonrakerJobControl`/`PrinterBridge` calls above, publishing `hydra/bridges/printer3d/state` (retained) and one `.../cmd/<verb>/result` per command - no new physical authority, only a new way to reach what already existed. `start` accepts `{"job": <job_to_dict>, "filename": ...}`; `job` accepts a bare `BridgeJob` for a gate-only check. Dispatch is pure enough to unit-test against the same local `ThreadingHTTPServer` Moonraker fixture `test_moonraker.py` already uses, no real broker required - `paho-mqtt` (optional `[mqtt]` extra) is only imported, lazily, inside `run_forever()`.
